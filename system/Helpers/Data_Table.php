...
class Data_Table {
    ...
    /**
     * Add a row containing the data.
     * @param array[string] $data
     * @return int Row ID
     */
    function addRow($data, $allowHtml = false) {
        ...
        // Assign the data to a cell.
        foreach ($data as $key => $value) {
            $this->td[$this->row][$key] = &$this->tr[$this->row]->newChild("td", $value, true, $allowHtml);
        }

        if (count($data) < $this->columns && $this->autoFill === true) {
            // Create the rest of the cells.
            for ($i = count($data); $i < $this->columns; $i++) {
                $this->td[$this->row][$key] = $this->tr[$this->row]->newChild("td", "", true, $allowHtml);
            }
        }
